...
def plot(params, set='test', x_axis='iter'):
    ...
    for lambda_l2 in metric2:
        

        acc2 = metric2[lambda_l2][:, t, 0] ## acc on: t=1 test, t=0 train

        bce2 = metric2[lambda_l2][:, t, 1] ## bce on: t=1 test, t=0 train
        
        if x_axis == 'iter':
            x2 = [i for i in range(time2[lambda_l2].shape[0])]
        ...
        fig_acc.add_trace(go.Scatter(x=x2, y=acc2,
                    mode='lines',
                    name=f'2nd, lambda_l2 = {lambda_l2}'))

        fig_bce.add_trace(go.Scatter(x=x2, y=bce2,
                            mode='lines',
                            name=f'2nd, lambda_l2 = {lambda_l2}'))

    for lambda_l2 in metric3:

        acc3 = metric3[lambda_l2][:, t, 0] ## acc on: t=1 test, t=0 train

        bce3 = metric3[lambda_l2][:, t, 1] ## bce on: t=1 test, t=0 train
        
        if x_axis == 'iter':
            x3 = [i for i in range(time3[lambda_l2].shape[0])]
        ...
        fig_acc.add_trace(go.Scatter(x=x3, y=acc3,
                    mode='lines',
                    name=f'3rd, lambda_l2 = {lambda_l2}'))
