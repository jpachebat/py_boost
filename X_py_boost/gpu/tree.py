...
class DepthwiseTreeBuilder:
    ...
    def __init__(self, borders, order, solution,
                 use_hess=True,
                 colsampler=None,
                 subsampler=None,
    ...
    def build_tree(self, X, grad, hess, grad3, grad4, sample_weight=None, grad_fn=None, *val_arrays):
        ...
        if sample_weight is not None:
            ...
            # Sample for Grad3
            grad3 = grad3 * sample_weight
            grad4 = grad4 * sample_weight
        ...
        for n_grp, grp_indexer in enumerate(output_groups):
            ...
            if self.order == 3:
                G3 = grad3[:, grp_indexer]
                G4 = None

            if self.order == 4:
                G3 = grad3[:, grp_indexer]
                G4 = grad4[:, grp_indexer]

            group_index[grp_indexer] = n_grp
            # grow single group of the tree and get nodes index
            train_nodes, valid_nodes = depthwise_grow_tree(tree, n_grp, X, G, H, G3, G4, self.order, self.solution,
        ...
        values = calc_node_values(grad, hess, grad3, grad4, self.order, self.solution, leaves, row_indexer, group_index, max_leaves, self.params['lr'],
                                  lambda_l2=self.params['lambda_l2'], lambda_l2_4=self.params['lambda_l2_4'])
