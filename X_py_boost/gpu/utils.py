...
from .loss_kernels import loss_kernel2, loss_kernel3, loss_kernel4, loss_kernel3_minus 
...
def histogram(arr, ghg4, nodes, col_indexer, row_indexer, out_indexer, nnodes, max_bins,
    ...
    fill_histogram(res, arr, ghg4, nodes, col_indexer, row_indexer, out_indexer)
...
def calc_loss(grad, hess, grad3, grad4, order, solution, nodes_count, lambda_l2=0.1, lambda_l2_4=[1, 0, 0], min_data_in_leaf=10.):
    ...
    if order == 3:
        loss_kernel3(grad, hess, grad3,
                    grad[..., -1:], hess[..., -1:], grad3[..., -1:],
                    grad[..., :1], hess[..., :1], grad3[..., :1],
                    nodes_count[cp.newaxis, :, cp.newaxis, cp.newaxis],
                    lambda_l2, min_data_in_leaf, res)
        ...
        """
        if solution == 'minus':
            loss_kernel3_minus(grad, hess, grad3,
                        grad[..., -1:], hess[..., -1:], grad3[..., -1:],
                        grad[..., :1], hess[..., :1], grad3[..., :1],
                        nodes_count[cp.newaxis, :, cp.newaxis, cp.newaxis],
                        lambda_l2, min_data_in_leaf, res)
        ...
        if solution == 'plus':
            loss_kernel3exact_plus(grad, hess, grad3,
                        grad[..., -1:], hess[..., -1:], grad3[..., -1:],
                        grad[..., :1], hess[..., :1], grad3[..., :1],
                        nodes_count[cp.newaxis, :, cp.newaxis, cp.newaxis],
                        lambda_l2, min_data_in_leaf, res)
...
"""
    if order == 4:
        loss_kernel4(grad, hess, grad3, grad4,
                    grad[..., -1:], hess[..., -1:], grad3[..., -1:], grad4[..., -1:],
                    grad[..., :1], hess[..., :1], grad3[..., :1], grad4[..., :1],
                    nodes_count[cp.newaxis, :, cp.newaxis, cp.newaxis],
                    lambda_l2_4[0], lambda_l2_4[1], lambda_l2_4[2], min_data_in_leaf, res)
...
def depthwise_grow_tree(tree, group, arr, grad, hess, grad3, grad4, order, solution, row_indexer, col_indexer, params, valid_arrs=None):
    if valid_arrs is None:
        valid_arrs = []

    # create gh
    n_out = grad.shape[1]
    # Concatenate also Grad3, Grad4
    if order == 2:
        ghg4 = cp.concatenate((grad, hess), axis=1)
    if order == 3:
        ghg4 = cp.concatenate((grad, hess, grad3), axis=1)
    if order == 4:
        ghg4 = cp.concatenate((grad, hess, grad3, grad4), axis=1)
    out_indexer = cp.arange(ghg4.shape[1], dtype=cp.uint64)
    ...
    for niter in range(params['max_depth']):
        ...
        ghg4_hist = histogram(arr, ghg4, node_indexes,
        ...
        if order == 3:
            g3 = ghg4_hist[n_out*2:n_out*3]
            g4 = None
            
        if order == 4:
            g3 = ghg4_hist[n_out*2:n_out*3]
            g4 = ghg4_hist[n_out*3:]
        
        # assume hess is the last output
        loss = calc_loss(g1, g2, g3, g4, order, solution, nodes_count, lambda_l2=params['lambda_l2'], lambda_l2_4=params['lambda_l2_4'],
        ...
        if niter < (params['max_depth'] - 1):
            ...
            prev_hist, small_index, big_index = get_prev_hist(cpu_counts,
                                                              ghg4_hist, cp.asarray(is_valid_node))

    return nodes, valid_nodes

accumulate_gh_kernel2 = cp.ElementwiseKernel(
    """
    uint64 row_indexer,
    uint64 group_index, 
...
accumulate_gh_kernel3 = cp.ElementwiseKernel(
    """
    uint64 row_indexer,
    uint64 group_index, 
    ...
    """,
    'raw float32 grad_sum, raw float32 hess_sum, raw float32 grad3_sum',
    ...
    atomicAdd(&grad_sum[y_ptr], grad[row_indexer * nout + out]); 
    atomicAdd(&hess_sum[y_ptr], hess[row_indexer * hxst + out * hyst]); 
    atomicAdd(&grad3_sum[y_ptr], grad3[row_indexer * nout + out]);
...
accumulate_gh_kernel4 = cp.ElementwiseKernel(
    """
    uint64 row_indexer,
    uint64 group_index, 
    ...
    """,
    'raw float32 grad_sum, raw float32 hess_sum, raw float32 grad3_sum, raw float32 grad4_sum',
    ...
    atomicAdd(&hess_sum[y_ptr], hess[row_indexer * hxst + out * hyst]); 
    atomicAdd(&grad3_sum[y_ptr], grad3[row_indexer * nout + out]);
    atomicAdd(&grad4_sum[y_ptr], grad4[row_indexer * nout + out]); 
...
def calc_node_values(grad, hess, grad3, grad4, order, solution, nodes, row_indexer, group_index, max_nodes, lr=1, lambda_l2=0.1, lambda_l2_4=[1, 0, 0]):
    ...
    if order == 2:
        ...
        accumulate_gh_kernel2(row_indexer[:, cp.newaxis], group_index[cp.newaxis, :], grad, hess, nodes,
    ...
    if order == 3:
        ...
        grad_sum = cp.zeros((max_nodes, nout), dtype=cp.float32)
        hess_sum = cp.zeros((max_nodes, nout), dtype=cp.float32)
        grad3_sum = cp.zeros((max_nodes, nout), dtype=cp.float32)

        accumulate_gh_kernel3(row_indexer[:, cp.newaxis], group_index[cp.newaxis, :], grad, hess, grad3, nodes,
                            nout, ngroups, hess.shape[1], int(hess.shape[1] > 1),
                            grad_sum, hess_sum, grad3_sum)

        node_values = - 2 * grad_sum * (hess_sum + lambda_l2) / (2 * (hess_sum + lambda_l2) ** 2 - grad_sum * grad3_sum)
        """
        if solution == 'approximation':
            ...
            node_values = - grad_sum * lr / (hess_sum + lambda_l2) * ( 1 + grad_sum * grad3_sum/ (2 * (hess_sum + lambda_l2) ** 2) )
...
"""
    if order == 4:
        ...
        hess_sum = cp.zeros((max_nodes, nout), dtype=cp.float32)
        grad3_sum = cp.zeros((max_nodes, nout), dtype=cp.float32)
        grad4_sum = cp.zeros((max_nodes, nout), dtype=cp.float32)

        accumulate_gh_kernel4(row_indexer[:, cp.newaxis], group_index[cp.newaxis, :], grad, hess, grad3, grad4, nodes,
                            nout, ngroups, hess.shape[1], int(hess.shape[1] > 1),
                            grad_sum, hess_sum, grad3_sum, grad4_sum)

        node_values = - grad_sum * ((hess_sum + lambda_l2_4[0]) * (hess_sum + lambda_l2_4[0]) - grad_sum * (grad3_sum + lambda_l2_4[1]) / 2) / ((hess_sum + lambda_l2_4[0]) * (hess_sum + lambda_l2_4[0]) * (hess_sum + lambda_l2_4[0]) - grad_sum * (hess_sum + lambda_l2_4[0]) * (grad3_sum + lambda_l2_4[1]) + (grad4_sum + lambda_l2_4[2]) * grad_sum * grad_sum / 6)
